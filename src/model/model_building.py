...
def apply_tfidf(train_data: pd.DataFrame, max_features: int, ngram_range: tuple) -> tuple:
    ...
    try:
        vectorizer = TfidfVectorizer(max_features=max_features, ngram_range=ngram_range, min_df=5, max_df=0.95)
...
# FIXED: Added num_leaves: int to the function signature
def train_lgbm(X_train: np.ndarray, y_train: np.ndarray, learning_rate: float, max_depth: int, n_estimators: int, num_leaves: int) -> lgb.LGBMClassifier:
...
def main():
    try:
        ...
        best_model = train_lgbm(X_train_tfidf, y_train, learning_rate, max_depth, n_estimators, num_leaves)
